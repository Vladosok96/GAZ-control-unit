...
void LCD::collectFaults(bool rcRemote, bool wireRemote, bool stopButton) {
    if (!rcRemote)
        LCD::rcRemote = true;
    if (!wireRemote)
        LCD::wireRemote = true;
        ...
        switch (current_display_page) {
            ...
            case CONTROL_MODE:
                ...
                switch (StateMachine::get_current_control_mode()) {
                    ...
                    case StateMachine::Mode::Disable:
                        sprintf(display_buffer, "ОТКЛЮЧЕНО");
                        display.drawUTF8(38, 29, display_buffer);
                        break;
                    default:
                        sprintf(display_buffer, "НЕИЗВЕСТНО");
                        display.drawUTF8(38, 29, display_buffer);
                        break;
                }
            ...
            case MESSAGES:
                ...
                */
                ...
                current_display_page = FAULTS;
                break;
            case FAULTS:
                display.drawButtonUTF8(64, 10, U8G2_BTN_HCENTER | U8G2_BTN_INV, 124, 3, 3, "История ошибок");

                sprintf(display_buffer, rcRemote ? "1. ПУЛЬТ FlySky:FAIL" : "1. ПУЛЬТ FlySky:  OK");
                display.drawUTF8(4, 25, display_buffer);

                sprintf(display_buffer, wireRemote ? "2. СВЕТОФОР:    FAIL" : "2. СВЕТОФОР:      OK");
                display.drawUTF8(4, 37, display_buffer);

                sprintf(display_buffer, stopButton ? "3. E-Stop:      FAIL" : "3. E-stop:        OK");
                display.drawUTF8(4, 49, display_buffer);
